...
/**
 * Calcula Juros Compostos Semestral de forma dinâmica.
 * 
 * Aplica a fórmula: (JUROS/100+1) ^ (QTDE MESES/2)
 * O expoente é calculado dinamicamente como QTDE MESES/2
 */
function calcularJurosCompostosSemestral($valor, $taxaSemestral, $dias, $diasBase = 30)
{
    // Taxa em formato decimal
    $taxaDecimal = $taxaSemestral / 100;

    // Calculamos a quantidade de meses
    $qtdeMeses = $dias / $diasBase;

    // Calculamos a quantidade de semestres (contagem para exibição)
    ...
    return [
        ...
        'qtde_semestres' => $qtdeSemestres,
        'expoente' => $expoente,  // Expoente da fórmula (qtdeMeses/2)
        'taxa_semestral' => $taxaSemestral,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $taxaSemestral = isset($_POST['taxa_juros']) ?
    ...
    if (!empty($dataInicio) && !empty($dataFim) && $valorBase > 0 && $taxaSemestral > 0) {
        ...
        // Calcula os juros compostos semestrais
        $resultado = calcularJurosCompostosSemestral($valorCorrigido, $taxaSemestral, $dias, $diasBase);
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Juros Compostos Semestral</title>
...
<body class="bg-light p-4">
    <div class="container bg-white p-4 rounded shadow-sm">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0">JUROS SEMESTRAIS COMPOSTOS</h4>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="tipo_juros" class="form-select" disabled>
                        <option selected>Juros Semestrais Compostos</option>
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Taxa de Juros Semestral (%):</label>
        ...
        <?php if ($resultado): ?>
            <div class="mt-4">
                <h5>JUROS SEMESTRAIS COMPOSTOS</h5>
                ...
                <p><strong>JUROS:</strong> <?= number_format($resultado['taxa_semestral'], 2, ',', '.') ?></p>
                <p><strong>TOTAL JUROS:</strong> <?= number_format($resultado['qtde_meses'], 2, ',', '.') ?></p>
                <p><strong>SEMESTRAL:</strong> <?= number_format($resultado['qtde_semestres'], 0, ',', '.') ?></p>

                <h5 class="mt-3 bg-yellow">FÓRMULA</h5>
                <p><strong>FÓRMULA:</strong> (JUROS/100+1) ^ (<?= number_format($resultado['expoente'], 2, ',', '.') ?>)</p>
                <p><strong>VALOR DA 1A. PARTE FÓRMULA:</strong> (<?= number_format($resultado['taxa_semestral'], 2, ',', '.') ?> / 100 + 1 = <?= number_format($resultado['fator_1'], 4, ',', '.') ?>)</p>
                <p><strong>COMO FICA A FÓRMULA:</strong> <?= number_format($resultado['fator_1'], 4, ',', '.') ?> ^ <?= number_format($resultado['expoente'], 2, ',', '.') ?></p>
